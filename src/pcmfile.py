...
class PCMFile(AudioFile):
    ...
    def ReadFileHeader(self):
        ...
        tag = self.fp.read(12)
        if tag[0:4] != b"RIFF" or tag[8:12] != b"WAVE":
            raise RuntimeError("ERROR: File opened for PCMFile is not a RIFF file!")
        # find format chunk
        while True:
            tag = self.fp.read(4)
            if len(tag) < 4:
                raise RuntimeError(
                    "ERROR: Didn't find WAV file 'fmt ' chunk following RIFF file header"
                )
            if tag[0:4] == b"fmt ":
                break
        ...
        (
            ...
            blockAlign,
            bitsPerSample,
        ) = unpack(b"<LHHLLHH", tag)
        if formatTag != 1:
            raise IOError("Opened a non-PCM WAV file as a PCMFile")
        if bitsPerSample != 16:
            raise RuntimeError("PCMFile was not 16-bits per sample")
        # find data chunk
        to_read = 4
        while True:
        ...
        assert bitsPerSample // BYTESIZE == bitsPerSample / BYTESIZE, (
            "PCMFile bitsPerSample was not integer number of bytes"
        )
        numSamples //= nChannels * (
            bitsPerSample // BYTESIZE
        )  # assumes bitsPerSample is integer number of bytes
    ...
    def ReadDataBlock(self, codingParams):
        ...
        bytesToRead = (
            ...
            * (codingParams.bitsPerSample // BYTESIZE)
        )
        if (
            codingParams.nChannels
            * codingParams.numSamples
            * (codingParams.bitsPerSample // BYTESIZE)
            - codingParams.bytesReadSoFar
            <= 0
        ):
            dataBlock = None
        elif (
        ...
        #            dataBlock=np.fromstring(dataBlock,dtype=np.int16)  # uses Local Endian conversion -- use byteswap() method if wrong Endian
            ...
            raise IOError("PCMFile was not 16-bit PCM in PCMFile.ReadDataBlock!")
    ...
    def WriteDataBlock(self, data, codingParams):
        ...
        for iCh in range(nChannels):
            ...
            temp[signs] *= -1.0  # now temp is positive
            temp = vQuantizeUniform(
                temp, 16
            )  # returns 16 bit quantized codes (stored as unsigned ints)
            temp = temp.astype(
                np.int16
            )  # now it can take a 2s complement sign (it was unsigned before)
